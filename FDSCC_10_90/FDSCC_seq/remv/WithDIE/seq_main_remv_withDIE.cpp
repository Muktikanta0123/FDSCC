...
enum type {ADDV, ADDE, REME, REMV, CONV, CONE, COUNT};
	...
	//else optype = 1;
      ...
      case ADDE : 
                     // fscanf(fp, "%d", &u);
                      ...
                      res = G1.AddGraphEdge(u,v, vertexID+2);
                      //cout<<" a(u,v):"<<"("<<u<<","<<v<<")";
                       //pthread_mutex_unlock(&lock);
                       if(res == true)
                       naddE++;
     ...
     case REME : 
                      ...
                      res = G1.RemoveGraphEdge(u,v, vertexID+2);  
                      //cout<<" r(u,v):"<<"("<<u<<","<<v<<")";                      
                       //pthread_mutex_unlock(&lock);
                       if(res == true)
                       nremE++;
        ...
        case REMV :
                     ...
                     u = (rand() % (vertexID));
                      ...
                      res = G1.RemoveSCCV(u, vertexID+2);
                      if(res == true)
                       nremV++;
                       else nremV1++;
                      // pthread_mutex_unlock(&lock);
