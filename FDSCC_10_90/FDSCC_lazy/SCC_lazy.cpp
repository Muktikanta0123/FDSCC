...
class SCC{
     ...
     public:
        ...
        cclist_t* createNewCCNodeWithOldVertex(slist_t * oldV, int cc_num){
         ...
         void locateV(slist_t* head, slist_t* tail, slist_t ** pred, slist_t ** curr, int key){
               (*pred)=head;
...
//                  cout<<"before insert curr->val, pred and ccno:"<<curr1->val<<" "<<pred1->val<<" "<<cc->ccno<<endl;
               ...
               // slist_t*p = oldV->vnext;
                ...
                //pthread_mutex_unlock(&(pred1)->lock);    // unlock pred
                // pthread_mutex_unlock(&(curr1)->lock);    // unlock curr
                 return;
                }
                else{
                //pthread_mutex_unlock(&(pred1)->lock);    // unlock pred
     ...
     /*it will return the reference to the pred & curr cc and the references to the pred & curr vertex having val=key*/
         bool findCCLocate(cclist_t **n1, cclist_t** n2, slist_t **pred, slist_t ** curr, int key){
	        ...
	        // pthread_mutex_lock(&temp1->lock);	         
	         while(temp1 != CCTail){
        	        // pthread_mutex_lock(& temp1->vnext.load(memory_order_seq_cst)->lock);	                
		        temp2 = temp1->vnext.load(memory_order_seq_cst);
		        temp3 = temp2->vnext.load(memory_order_seq_cst);
        	        // pthread_mutex_lock(&(temp3)->lock);		        
       ...
       /*wait-free cntains v*/
        ...
        l1:     pthread_mutex_lock(&(CCHead)->lock); // lock the CCHead
                cclist_t * curr = CCHead->next.load(memory_order_seq_cst);
                pthread_mutex_lock(&curr->lock);   // lock the CCHead->next           
             if(curr->marked.load(memory_order_seq_cst) == false && CCHead ->next.load(memory_order_seq_cst) == curr){ // check for validation
        ...
        bool AddVertexCC(int key){
         ...
         bool RemoveVertexCC(int key){
                ...
                if(curr->val == key){ // if vertex is present just remove it 
                       pred ->vnext.store(curr->vnext.load(memory_order_seq_cst),memory_order_seq_cst); // physical delete
                       // pred ->vnext.curr->vnext.load(memory_order_seq_cst);   
                       pthread_mutex_unlock(&pred->lock);
                        pthread_mutex_unlock(&curr->lock); 
                        return true; // return true
                }
  ...
  /* it wll detele all empty CCs, means if no vertex present in a cc*/
    void RemoveCC(){
        // pthread_mutex_lock(&CCHead->lock);
  ...
  bool AddE(slist_t * EHead, int key){
                        ...
                        if(findVSameCC(slHead, &pred, &oldV, j)){ // vertex belongs to same cc
                            if(SUCC[j] != PREC[j]) // check for unprocessed vertex for next cc 
                                 ...
                                 if(st == 1){ // for the first vertex for the new cc node creation
                                    ...
                                    bool flag = findCCLocate(&predn2, &currn2, &pred2, &curr2, j);// find the location of the vertex in the vertex list
                                      /* create new cc with key curr->val and insert at the begining of CCHead  */
                                       ...
                                       if(cp->marked.load(memory_order_seq_cst) == false && CCHead->next.load(memory_order_seq_cst) == cp && ValidateV(pred2, curr2)){ // check for validation
                             ...
                             // cout<<"flag1 and curr1->val and currn1:"<<flag1<<" "<<curr1->val<<" "<<currn1->ccno<<endl;
                                  ...
                                  pthread_mutex_lock(&pred1->lock); // lock the pred1
                                  pthread_mutex_lock(&curr1->lock); // lock the curr1
                                  ...
                                  pthread_mutex_unlock(&pred1->lock); // lock the pred1
                                  pthread_mutex_unlock(&curr1->lock); // lock the curr1
                                }
        ...
        bool updateSCCAfterRemoeveE(cclist_t *predcc1, cclist_t *currcc1, cclist_t *predcc2, cclist_t *currcc2){
         ...
         if(currcc1->ccno != currcc2->ccno){
             ...
             bool flag = findSCCafterRemE(currcc1->vnext.load(memory_order_seq_cst), currcc1->ccno);
             if(flag == true){
...
//cout<<"s:"<<s<<endl;
         ...
         // if(flag1 == true && currn->ccno != cc2->ccno){
          temp1 = curr->enext.load(memory_order_seq_cst); // Ehead
          for(it = temp1->enext.load(memory_order_seq_cst); it->enext.load(memory_order_seq_cst) != NULL; it = it ->enext.load(memory_order_seq_cst)){ // iterate over each edges of the vertex it
...
/*Tarjan's algorithm to find the SCC*/
 void VISIT(slist_t *pred, slist_t *curr){
    ...
    if(Root[curr->val] == curr->val){
      ...
      changePtr(&pred2, &curr2);
       pthread_mutex_unlock(&pred2->lock);
       pthread_mutex_unlock(&curr2->lock);
       pthread_mutex_unlock(&predn2->lock);
       pthread_mutex_unlock(&currn2->lock);
      //PrintSCC();
       int t = ccCount +1; 
       ccCount.store(t);
      do{
     ...
     // cout<<"flag1 and curr1->val and currn1:"<<flag1<<" "<<curr1->val<<" "<<currn1->ccno<<endl;
      if(flag1 == true && currn1->ccno != newcc->ccno){
         ...
         pthread_mutex_unlock(&pred1->lock);
         pthread_mutex_unlock(&curr1->lock);
         pthread_mutex_unlock(&predn1->lock);
         pthread_mutex_unlock(&currn1->lock);  
      }while(w != curr->val);
    }
 }   
      ...
      VISIT(cc2->vnext.load(memory_order_seq_cst), temp2);
      /*
       while(temp2->vnext!=NULL){ // iterate over each vertex of cc2
        temp1 = temp2; //temp1 used for iteration of all vertices and edges in cc2  
        bool *  visited = new bool[n]; // create a visiting array
