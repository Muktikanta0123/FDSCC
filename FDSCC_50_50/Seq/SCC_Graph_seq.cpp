...
class list1{
 public:
        ...
        void locate(slist_t* head, slist_t* tail, slist_t ** n1, slist_t ** n2, long int key){
        ...
        bool Contains(slist_t * h, slist_t *t, slist_t ** n, long int key){
        ...
        bool Add(slist_t * h, slist_t *t, long int key){
                // allcate new node
                slist_t* pred, *curr;
                locate(h, t, &pred, &curr, key);
                if(h == Head){
    ...
    bool Remove(slist_t *h, slist_t *t, long int key){
        slist_t* pred, *curr;
        locate(h, t, &pred, &curr, key);
        if(h==Head){
                if(curr->val == key){
...
class SCC{
     ...
     public:
        ...
        cclist_t* createCCNode(long int key){
                /*sentinal */
        ...
        cclist_t* createNewCCNodeWithOldVertex(slist_t * oldV, long int cc_num){
                /*sentinal */
                slist_t* VHead = (slist_t*) malloc(sizeof(slist_t));
                VHead ->val = INT_MIN;
                VHead ->vnext = NULL;
         ...
         void locateV(slist_t* head, slist_t* tail, slist_t ** n1, slist_t ** n2, long int key){
                  //         cout<<"loc begins"<<endl;
                slist_t* pred = head; 
                slist_t* curr = pred->vnext; 
                while(curr != tail && curr->val < key){
...
//                  cout<<"before insert curr->val, pred and ccno:"<<curr1->val<<" "<<pred1->val<<" "<<cc->ccno<<endl;
        ...
        /*finds the cc present in the graph, returns the reference of that cc */
         bool findCC(cclist_t **n, long int key){
     ...
     /*it will return the reference to the pred & curr cc and the references to the pred & curr vertex having val=key*/
         bool findCCLocate(cclist_t **n1, cclist_t** n2, slist_t **pred, slist_t ** curr, long int key){
        ...
        bool AddCC(long int key){
         ...
         bool AddVertex(slist_t * Vhead, long int key){
        ...
        /* Add vertex in the CC list of VHead(h) in the stored position */
         ...
         bool RemoveVertexCC(long int key){
  ...
  /* it wll detele all empty cc, means if no vertex present in a cc*/
    ...
    void locateE(slist_t* EHead, slist_t ** n1, slist_t ** n2, long int key){
                  //         cout<<"loc begins"<<endl;
                slist_t* pred = EHead; 
  ...
  bool AddE(slist_t * EHead, long int key){
           slist_t* pred, * curr;
        ...
        bool RemoveE(slist_t * EHead, long int key){
           slist_t* pred, * curr;
            locateE(EHead, &pred, &curr, key);
            if(curr->val == key){
                        pred ->enext = curr->enext;
        ...
        bool findVSameCC(slist_t *slHead, slist_t**pred, slist_t **curr, long int key){
        ...
        void findVInCC(slist_t *slHead, long int j, slist_t **oldV){
        ...
        void DFSFW(slist_t * slHead, slist_t* it_edge, long int num_cc, long int cc){
        ...
        /*DFS of the forward traversal*/      
        void DFSBW(slist_t * slHead, slist_t* it_edge, long int num_cc, long int cc){
         slist_t * curr, *pred;//used for iteration of each vertex in findVSameCC
        ...
        bool findSCCafterRemE(slist_t * slHead, long int cc){
         
          long int num_cc =  ccid;// getting last cc id of SCC
         ...
         // cclist_t *newcc;
          ...
          for(it = slHead->vnext; it->vnext != NULL; it = it ->vnext){
            ...
            // cout<<"SUCC[it->val] and PREC[it->val]:"<<  SUCC[it->val] <<" "<< PREC[it->val]  <<endl;
                    long int st = 1; // test for the first vertex of the SCC
                    for(long int j = 1; j <= MAX ; j++) { // iterate over all vertices            
           ...
           bool RemoveGraphEdge(long int key1, long int key2){
      ...
      //  if(checkEqual(cc1, cc2)) // base case, only for two CCs
       ...
       //cout<<"temp2:"<<temp2->val<<endl;
             //long int p;cout<<"hello"<<p<<endl; cin>>p;           
        temp1 = temp2; //temp1 used for iteration of all vertices and edges in cc2  
        bool *  visited = new bool[n]; // create a visiting array
        for(long int i =0; i < n; i++) // intially all vertices are false
                visited[i] = false;
        list<long int> que; // queue for processing the reachablity
...
//cout<<"s:"<<s<<endl;
            long int s = que.front(); // get the front item of the que
...
bool flag2 = checkVPresentInCC(cc1, it->val);//checks whether s is present in the cc1 
   ...
   //long int p;cout<<"reach"<<reach<<endl; cin>>p;
     //cout<<"reach:"<<reach<<endl;
      ...
      bool checkVPresentInCC(cclist_t * cc1, long int key){
      ...
      long int Stk[MAX]; // stack used in the Tarjan's algrithm
      long int Root[MAX];//used to hold the root vertex of the SCC
      long int Comp[MAX];//stores the component number of the vertex,
      bool visited[MAX];// stores the status of the each vertex, visited or not
      long int top=-1;
      void push(long int v){
       Stk[++top] = v;
      }  
      long int pop(){
        if(top == -1){
     ...
     // cout<<"flag1 and curr1->val and currn1:"<<flag1<<" "<<curr1->val<<" "<<currn1->ccno<<endl;
      ...
      /*
       while(temp2->vnext!=NULL){ // iterate over each vertex of cc2
        ...
        bool AddGraphEdge(long int key1, long int key2){
          top = -1;
                 slist_t* pred1, *curr1, *pred2, *curr2;
                cclist_t *predn1, *currn1, *predn2, *currn2;
                bool flag1 = findCCLocate(&predn1, &currn1, &pred1, &curr1, key1);
        ...
        bool ContainsE(long int key1, long int key2){
                slist_t* pred1, *curr1, *pred2, *curr2;
                cclist_t *predn1, *currn1, *predn2, *currn2;
                bool flag1 = findCCLocate(&predn1, &currn1, &pred1, &curr1, key1);
                bool flag2 = findCCLocate(&predn2, &currn2, &pred2, &curr2, key2);
...
//	 }   
   ...
   void create_initial_vertices(long int n){
          long int i=0;
          for(;i<n;i++){
            AddVertexCC(i);
          }
...
class CCGraph{
 ...
 public:      
      ...
      Graph(){
       ...
       // lt.init();
        bool AddV(long int key){
           return lt.Add(lt.Head,lt.Tail,key);
        }
        bool RemoveV(long int key){
        ...
        //void locateV(slist_t ** n1, slist_t ** n2, long int key);
        bool AddE(long int key1, long int key2){
          slist_t *u,*v;
        ...
        bool RemoveE(long int key1, long int key2){
         slist_t *u,*v;
          bool flag1 = lt.Contains(lt.Head, lt.Tail, &u, key1);
          bool flag2 = lt.Contains(lt.Head, lt.Tail, &v, key2);
          if(flag1 == false || flag2 == false)
